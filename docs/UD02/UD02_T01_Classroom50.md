...
- Estar unidos a la clase (Classroom): https://classroom50.org/IES-Eduardo-Primo-Marques/1dam-programacio-java/onboard
- Tener el IDE IntelliJ instalado en nuestro ordenador

## Aceptar el Assignement (tarea para la UD02)

Aceptamos el *Assignement* (la tarea/ejercicio) a partir del link del profesor, en este caso: https://classroom50.org/IES-Eduardo-Primo-Marques/1dam-programacio-java/assignments/ud02

La página nos muestra el nombre de la tarea, la fecha de entrega si la tiene, y con qué cuenta estamos identificados. Pulsamos el botón **Accept assignment**.

Al aceptar, Classroom 50 crea automáticamente nuestro repositorio privado en GitHub. Cuando termine, pulsamos **Open repository** para ir a nuestro repositorio en GitHub (ese es el que necesitamos para el siguiente paso).
...
**Recomendamos usar siempre SSH y no HTTPS.** Con SSH, una vez configurada la clave, `git push` funciona sin que GitHub te pida usuario, contraseña ni tokens cada vez — es un paso único por ordenador y evita tener que lidiar con tokens que caducan.
...
Ahora, en la página de nuestro repositorio en GitHub, pulsamos el botón verde **Code**, elegimos la pestaña **SSH** (no HTTPS) y copiamos esa URL:

![repositorio](assets/url_repository.png)

Abriremos IntelliJ y crearemos un nuevo proyecto (New Project from Version Control, o Project/Clone Repository) a partir de la url SSH que hemos copiado:
...
Debes tener en cuenta que:
  ...
  4. Solo se conceden puntos en Classroom 50 si todos los test de la clase funcionan. En este caso solo la clase `_01_Temperatura` nos otorgará puntos

## Subir nuestra solución a Classroom 50

Una vez comprobado que pasamos los tests lo que queda es subir nuestra solución para que sea evaluada y valorada por el docente (Podemos repetir el proceso tantas veces como queramos, así que lo podemos ir haciendo segun vayamos realizando los ejercicios). No hay un botón de "entrega" aparte: cada `push` a `main` dispara la corrección automáticamente.
...
## Ver tu nota y el desglose

Unos segundos/minutos después de cada push se ejecuta la corrección con GitHub Actions. Cada entrega genera una **Release** en tu repositorio (pestaña *Releases*), con la puntuación total y el resultado de cada test:

![Release con el desglose de la nota](assets/classroom50_release_resultado.jpg)

Cada fila es una clase de ejercicio; solo se dan los puntos de una fila si **todos** los tests de esa clase pasan — no hay puntos parciales dentro de un mismo ejercicio.

Al aceptar la tarea también se abre automáticamente un Pull Request llamado **Feedback**, que dura todo el curso (no lo cierres ni lo fusiones). Es donde tu profesor puede dejarte comentarios sobre tu código:

![Pull Request de Feedback](assets/classroom50_feedback_pr.jpg)
...
Debes enviar tus soluciones a Classroom 50 y superar al menos la mitad de los tests, cuantos más tests superados, mejor nota tendrás en la tarea.
